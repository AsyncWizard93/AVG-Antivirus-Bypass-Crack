########################################################################################################
#                                                                                                      #
# File Name: AVG-Antivirus-Premium-Reset.ps1     # Output: Schedule a task that resets the              #
# Author: Ammar S.A.A                           # AVG Antivirus Premium trial by changing the           #
...
$taskName = "AVG Antivirus-Premium-Reset"
$taskPath = "\"
$taskDescription = "A task that resets the AVG Antivirus Premium trial by changing the MachineGuid registry value"
...
switch ($taskRunResult.LastTaskResult) {
    0 {
        Write-Host "Last Run Status: AVG Antivirus Trial Was Resetted Successfully."
    }
    267011 {
        Write-Host "Status: Task has never ran but exists."
    }
    default {
        Write-Host "Last Run Status: AVG Antivirus Trial Reset Failed. Last Result: $($taskRunResult.LastTaskResult)"
    }
}
